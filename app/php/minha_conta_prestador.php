<?php
	include "header.php";
	$id = $_GET['id_usuario'];
	$sql = "select nome_prestador,foto_prestador from jobexpert.prestador where id_prestador = ".$id;
	$res = mysql_query($sql);

	$sql4 = "select avg(nota) from jobexpert.avaliacoes where prestador_id = ".$id.";";
	...
	if($res == true && $res4 == true){	
		...
		$my = array("nome"=>utf8_encode($row[0]),"foto"=>$row[1],"avaliacao"=>(string)$avaliacao);
	}	
	echo json_encode($my);
	mysql_close();
?>